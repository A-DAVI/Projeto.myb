...
export function initTextBindings(): void {
  const textMap: Array<{ inputId: string; setFn: (val: string) => void; renderIds: string[] }> = [
    {
      inputId: 'handle',
  ...
  textMap.forEach(({ inputId, setFn, renderIds }) => {
    ...
    input.addEventListener('input', () => {
      ...
      // casos especiais
      if (inputId === 'handle') applyHandle(input.value)
      if (inputId === 'f3-cta') applyCTA(input.value)
    })
  })
}
