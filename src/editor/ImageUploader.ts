...
const targets: Record<Slot, () => HTMLElement | null> = {
  1: () => document.getElementById('f1-pet'),
  2: () => document.getElementById('f2-deco'),
  3: () => document.getElementById('f3-deco'),
}

function applyImage(slot: Slot, dataUrl: string | null): void {
  const el = targets[slot]()
  if (!el) return
  el.style.backgroundImage = dataUrl ? `url(${dataUrl})` : ''
  if (slot === 1) el.classList.toggle('has-image', dataUrl !== null)
}

function warnIfSmall(file: File): void {
...
function readFile(file: File, slot: Slot): void {
  warnIfSmall(file)
...
export function initImageUploaders(): void {
  const slots: Slot[] = [1, 2, 3]

  slots.forEach(slot => {
    const fileInput = document.getElementById(`img-${slot}`) as HTMLInputElement | null
    ...
    if (dropZone) {
      ...
      dropZone.addEventListener('drop', e => {
        ...
        if (file && file.type.startsWith('image/')) readFile(file, slot)
      })
    }
...
export function restoreImages(images: Record<1 | 2 | 3, string | null>): void {
  ([1, 2, 3] as Slot[]).forEach(slot => {
    if (images[slot]) applyImage(slot, images[slot])
  })
}
